...
// Animate Hi Bubble with Mouse Hover/Leave
const useHover = () => {
	const avatar = useRef()
	...
	useEffect(() => {
		avatar.current.addEventListener('mouseenter', enter)
		avatar.current.addEventListener('mouseleave', leave)

		return () => {
			avatar.current.removeEventListener('mouseenter', enter)
			avatar.current.removeEventListener('mouseleave', leave)
		}
	}, [avatar])

	return [avatar, hovered]
}
	// const styles = { height: '90px'}
const Logo = () => {
	const [avatar, hovered] = useHover();

	// GSAP Animation Eye Blink: useEffect 
  const blink = useRef(null);  	

  useEffect(() => {
  	// Set Blink/Eye Transform Scale 
  	gsap.set(blink.current, {
  	...
  	const tl = gsap.timeline({
				  ...
				  tl.to(blink.current, { scaleY: .05})
				    .to(blink.current, { scaleY: 1});
  	}, []);

	return (
		<LogoWrap>
		<svg ref={avatar} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 285 230">
		  <g id="martin-avatar">
